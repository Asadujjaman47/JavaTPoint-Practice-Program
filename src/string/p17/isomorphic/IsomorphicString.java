...
public class IsomorphicString {
    ...
    public static boolean isIsomorphic(String str1, String str2) {
        ...
        for (int i = 0; i < str1.length(); i++) {
            ...
            char char_str1 = str1.charAt(i);
            char char_str2 = str2.charAt(i);

            // check whether a particular key (i-
            // th character) is being mapped into the Map or not
            if (map.containsKey(char_str1)) {
                if (map.get(str1) != char_str2)
                // returns false if the character of the first string is not equal to the character of the second string
                return false;
            }
            else {
                // checks whether a particular key (i-
                // th character) is being mapped into the Map or not
                if(map.containsKey(char_str2))
                    return false;
